...
def particle_locations(inp_dir, pp_tag, R1, R2, offset):
  ...
  sim_particles = []
  sim_particles.append([0., R1, R1, R1, R1])
  sim_particles.append([1., R1, 2. * R1 + R2 + offset, R1, R2])
...
def create_input_file(inp_dir, pp_tag):
  ...
  # from bulk modulus (power 6 for dimension 3)
  Kn_11 = 18. * get_eff_k(K1, K1) / (np.pi * np.power(horizon, 6))
  Kn_22 = 18. * get_eff_k(K2, K2) / (np.pi * np.power(horizon, 6))
  Kn_12 = 18. * get_eff_k(K1, K2) / (np.pi * np.power(horizon, 6))
  ...
  zone_1_circle = [R1, center[0], center[1], center[2]]
  inpf.write("    Parameters: " + print_dbl_list(zone_1_circle)) 

  ## zone 2 (top particle)
  inpf.write("  Zone_2:\n")
  inpf.write("    Is_Wall: false\n")
  inpf.write("    Type: sphere\n")
  zone_2_circle = [R2, center[0], center[1], center[2]]
  inpf.write("    Parameters: " + print_dbl_list(zone_2_circle)) 
  ...
  contain_params = [0., 0., 0., 2.*R1, 2.*R1 + 2.*R2 + particle_dist, 2.*R1]
  if R2 > R1:
    contain_params[3] = 2.*R2
    contain_params[5] = 2.*R2

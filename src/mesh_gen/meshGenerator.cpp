...
void buildGmshGeometryInCurrentModel(const std::string &geomName,
  ...
  if (geomName == "circle")
    throw std::runtime_error("buildGmshGeometryInCurrentModel: circle uses circleMeshSymmetric.");
...
void circleMeshSymmetric(const std::vector<double>& xc, double r, double h, const std::string& filename,
  ...
  if (symmetric_mesh) {
    ...
    gmsh::model::geo::synchronize();
    gmsh::model::mesh::generate(gmshMeshGenerateDim(modelDeck_for_out));
...
void generateSymmetricParticleMeshGmsh(const std::string &geomName,
  ...
  if (!known)
    throw std::runtime_error(
        "generateSymmetricParticleMeshGmsh: geometry type is not in geom::acceptable_geometries.");

  if (!geom::isNumberOfParamForGeometryValid(params.size(), geomName))
    throw std::runtime_error(
        "generateSymmetricParticleMeshGmsh: parameter count does not match geometry " + geomName + ".");

  if (geomName == "circle") {
    if (params.size() < 4)
      throw std::runtime_error(
          "generateSymmetricParticleMeshGmsh: circle needs radius and center (4 parameters).");
    circleMeshSymmetric({params[1], params[2], params[3]}, params[0], h, filenameStem, vtk_out, true,
                        write_mesh_file, out_mesh, meshDeck, modelDeck);
    return;
  }
  ...
  if (!write_mesh_file && out_mesh == nullptr)
    throw std::runtime_error(
        "generateSymmetricParticleMeshGmsh: when write_mesh_file is false, out_mesh is required.");

  if (out_mesh != nullptr) {
    if (meshDeck == nullptr || modelDeck == nullptr)
      throw std::runtime_error(
          "generateSymmetricParticleMeshGmsh: meshDeck and modelDeck are required when out_mesh is set.");
  ...
  if (write_mesh_file) {
    if (filenameStem.empty())
      throw std::runtime_error(
          "generateSymmetricParticleMeshGmsh: non-empty filename stem is required when writing a .msh file.");
  ...
  if (vtk_out) {
    if (filenameStem.empty())
      throw std::runtime_error(
          "generateSymmetricParticleMeshGmsh: non-empty filename stem is required for VTK output.");

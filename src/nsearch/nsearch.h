...
public:
  ...
  /*@brief name of tree: kdtree, octree, nflann_kdtree */
...
public:
  ...
  size_t radiusSearch(
    ...
    searchP.x = searchPoint.d_x;
    searchP.y = searchPoint.d_y;
    searchP.z = searchPoint.d_z;

    return d_tree.radiusSearch(searchP, search_r, neighs, sqr_dist);
  }
  ...
  size_t radiusSearch(
    ...
    std::vector<int> neighs_temp;
    std::vector<float> sqr_dist_temp;
    auto N =
    ...
    if (N > 0) {
      ...
      for (size_t i=0; i<N; i++) {
        neighs.push_back(size_t(neighs_temp[i]));
        sqr_dist.push_back(double(sqr_dist_temp[i]));
      }
    }
...
public:
  ...
  NSearchOct(size_t debug = 0, double tree_resolution = 1.)
      : BaseNSearch("octree", debug),
        d_cloud_p(new pcl::PointCloud<pcl::PointXYZ>), d_tree(tree_resolution){}

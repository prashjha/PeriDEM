...
std::vector<util::Point> getCornerPoints(
...
std::pair<util::Point, util::Point> &box) {
  ...
  if (dim == 1) {
    ...
    auto corner_pts = getCornerPoints(dim, box);
...
bool areBoxesNear(const std::pair<util::Point, util::Point>
    &b1,
...
bool isPointInsideRectangle(util::Point x, double x_min,
...
bool isPointInsideAngledRectangle(util::Point x, double x1,
                                                  double x2, double y1,
...
bool isPointInsideCylinder(const util::Point &p, const double &radius,
...
util::Point getPointOnLine(const util::Point &p1, const
util::Point &p2,
    const double &s) {
  return (1. - s) * p1 + s * p2;
}
